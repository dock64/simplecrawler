...
// fixing problem with raw deflate from IIS server
// https://stackoverflow.com/questions/37519828/how-can-we-distinguish-deflate-stream-from-deflateraw-stream
// https://en.wikipedia.org/wiki/HTTP_compression#Problems_preventing_the_use_of_HTTP_compression
// the normal zlib.unzip() does not detect if inflateRaw() is needed, so we have to do it.
var unzipAuto = function(buffer, callback) {
  if ((buffer[0] >> 4) & 8) {
        ...
        if (response.statusCode >= 200 && response.statusCode < 300) {
            ...
            response.on("end", function() {
                ...
                if (contentEncoding && /(gzip|deflate)/.test(contentEncoding)) {
                    unzipAuto(responseBuffer, decodeAndReturnResponse);
                } else {
        ...
        function processReceivedData() {
            ...
            crawler.queue.update(queueItem.id, {
                ...
                if (/(gzip|deflate)/.test(queueItem.stateData.headers["content-encoding"])) {
                    unzipAuto(responseBuffer, function(error, decompressedBuffer) {
                        if (error) {
                            /**
